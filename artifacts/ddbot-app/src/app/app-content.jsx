...
import { api_base, ApiHelpers, runIrreversibleEvents, ServerTime } from '@/external/bot-skeleton';
...
const AppContent = observer(() => {
    // Guest users (no auth token) bypass the loading screen entirely:
    // the chart renders immediately via ChartDataLayer's preview mode.
    // Authenticated users still show a brief loading screen while their
    // account data loads over WS, but with a hard 3-second cap.
    // RULE: use canonical auth check — no direct localStorage reads.
    const _hasToken = AuthSessionManager.isAuthenticated();
    const [is_api_initialized, setIsApiInitialized] = React.useState(!_hasToken);
    const [is_loading, setIsLoading] = React.useState(_hasToken);
    ...
    const changeActiveSymbolLoadingState = () => {
        ...
        if (ApiHelpers?.instance?.active_symbols) {
            ...
            // Poll until ApiHelpers instance is ready, then fetch active symbols.
            // NOTE: Do NOT reference is_loading here — it's a stale closure at call time.
            ...
            // Hard timeout — always force loading to false.
            // The original `if (is_loading)` check was a stale-closure bug:
            // setIsLoading(true) is async so is_loading is still false in this closure.
            setTimeout(() => {
                clearInterval(intervalId);
    ...
    React.useEffect(() => {
        if (is_api_initialized) {
            init();
            if (!client.is_logged_in) {
                // Guest: chart is already visible (ChartDataLayer preview mode).
                // Fetch active symbols in the background without a loading screen.
                setIsLoading(false);
                changeActiveSymbolLoadingState();
            } else {
                // Logged-in: brief loading while WS auth + account data loads.
                // Safety net clears it after 3 s if auth stalls.
                setIsLoading(true);
            }
            const safetyTimer = setTimeout(() => setIsLoading(false), 3_000);
    ...
    // Note: trade_definition_market.js handles its own deferred repopulation per-block
    // via EventBus('active_symbols:loaded'). Each block captures its _intended_value
    // (saved by the fromXml patch in field.js) and populates its own dropdowns directly
    // inside runInvisibleEvents so no BLOCK_CHANGE cascade fires with stale '' values.

    // Fallback for logged-in users: if is_landing_company_loaded never fires within 15 s
    // (WS auth stalled, OTP failed, etc.) force-load active symbols so the bot panel
    // does not stay in "Not available" state forever.
    React.useEffect(() => {
        if (!is_api_initialized || !client.is_logged_in) return;
        if (client.is_landing_company_loaded) return; // already handled above

        const fallback = setTimeout(() => {
            console.warn('[AppContent] is_landing_company_loaded fallback fired — forcing active symbols load');
            changeActiveSymbolLoadingState();
        }, 15_000);
