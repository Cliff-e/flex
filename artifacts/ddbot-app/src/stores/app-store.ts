...
export default class AppStore {
    ...
    onMount = async () => {
        ...
        // Guard: setDBotEngineStores() is normally called by AppContent.init() via useEffect,
        // but for guest users AppContent sets is_api_initialized=true immediately so BotBuilder
        // mounts and fires onMount() *before* the parent useEffect runs init().
        // Recover here so Blockly always initialises regardless of call order.
        if (!this.dbot_store) {
    ...
    registerOnAccountSwitch = () => {
        const onSocketOpened = (is_socket_opened: boolean) => {
        ...
        // MobX reaction() only fires on future *changes*. If the socket is already open
        // when this reaction is registered (common on first load), the callback would
        // never run and market dropdowns would remain unpopulated. Fire immediately.
