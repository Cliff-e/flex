...
export default class ClientStore {
    ...
    constructor() {
        // Pre-seed is_logged_in from canonical auth state so components that check
        // it (e.g. run-panel-store.onRunButtonClick) don't see "not logged in"
        // during the brief window between page load and WS authorize completing.
        // RULE: must NOT read localStorage directly — use AuthSessionManager.
        const isAuth = AuthSessionManager.isAuthenticated();
        // Only pre-seed when the logged_state cookie is not explicitly 'false'
        const loggedOutCookie =
            typeof document !== 'undefined'
                ? document.cookie.split(';').some(c => c.trim() === 'logged_state=false')
                : false;
        if (isAuth && !loggedOutCookie) {
    ...
    getCurrency = () => {
        // Use the MobX-tracked currency observable — no direct localStorage reads.
        return this.currency ?? '';
    };

    getToken = () => {
        // Read via AuthSessionManager — single source of truth for auth data.
    ...
    logout = async () => {
        // ── Step 1: Atomically clear in-memory auth state first ───────────────
        // This transitions canonical state to "not authorized" immediately so
        // that every subscriber (useApiBase, AuthReadyContext, bot-auth-guard)
        // sees isAuthorized=false before any localStorage key is removed.
        // The WebSocket stays open at this point so the WS logout() call below
        // can still reach the server.
        AuthSessionManager.clearSession();

        // ── Step 2: Signal all EventBus listeners (AuthManager, bots) ─────────
        // AuthManager listens for this to stop the proactive token-refresh cycle.
        // Bot guards listen for this to pause/stop any running strategy.
        EventBus.emit('auth:logout');

        // ── Step 3: Reset MobX observables ────────────────────────────────────
        this.account_list = [];
        ...
        // ── Step 5: Clear ancillary UI services ───────────────────────────────
        // disable livechat
        ...
        // ── Step 6: Clear localStorage (after WS is still open for logout call) ──
        ...
        // ── Step 7: Send WS logout request, then close the private socket ─────
        // Wrap in Promise.resolve() so the chain always runs even when
        // api_base?.api is null (optional chaining would otherwise short-circuit
        // the entire .then/.catch block, skipping disconnect and navigation).
        // The socket is still open here so the logout message can reach Deriv.
        let logoutError: Error | undefined;
        return Promise.resolve(api_base?.api?.logout())
            .catch((error: Error) => {
                console.error('Logout WS call failed:', error);
                logoutError = error;
            })
            .finally(() => {
                // Always disconnect the private socket after logout attempt,
                // regardless of WS logout success, failure, or missing api instance.
