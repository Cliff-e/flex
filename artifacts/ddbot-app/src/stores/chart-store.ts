...
export default class ChartStore {
    ...
    updateSymbol = () => {
        ...
        const raw = market_block?.getFieldValue('SYMBOL_LIST') ?? api_base?.active_symbols[0]?.symbol;
        // Guard: never assign an invalid symbol — fall back to R_100
        this.symbol = (raw && raw !== 'undefined') ? raw : 'R_100';
    };
    ...
    restoreFromStorage = () => {
        try {
            ...
            if (props) {
                ...
                // Guard against previously-stored "undefined" string or falsy values
                this.symbol = (symbol && symbol !== 'undefined') ? symbol : 'R_100';
                this.granularity = granularity;
                this.chart_type = chart_type;
            } else {
                this.symbol = 'R_100';
