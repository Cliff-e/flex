...
export default class ContractsFor {
    ...
    async getContractsFor(symbol) {
        ...
        const getContractsForFromApi = async () => {
            ...
            // The new trading API (api.derivws.com) omits `barrier_category` from
            // contracts_for responses. Derive it from `contract_category` using the
            // BARRIER_CATEGORIES mapping so all downstream callers (getDurations,
            // getBarriers, getContractsByTradeType, etc.) continue to work.
            // Note: the new API uses 'asian' but the config key is 'asians', so we
            // normalise that single mismatch explicitly.
            const { BARRIER_CATEGORIES } = config();
            const catToBarrierCat = {};
            Object.entries(BARRIER_CATEGORIES).forEach(([bc, cats]) => {
                cats.forEach(cat => { catToBarrierCat[cat] = bc; });
            });
            const normalized_contracts = filtered_contracts.map(contract => {
                if (contract.barrier_category) return contract;
                const lookup_cat =
                    contract.contract_category === 'asian' ? 'asians' : contract.contract_category;
                const barrier_category = catToBarrierCat[lookup_cat];
                return barrier_category ? { ...contract, barrier_category } : contract;
            });
