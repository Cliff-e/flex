...
const TradeAnimation = observer(({ className, should_show_overlay }: TTradeAnimation) => {
    ...
    // Track ChartDataLayer mode so the Run button converts to a login CTA
    // in preview mode — prevents guests from accidentally triggering bot trades.
    const [dataMode, setDataMode] = React.useState<'preview' | 'live'>(() => ChartDataLayer.mode);
    React.useEffect(() => {
        const unsub = EventBus.on('chart:mode_changed', ({ mode }) => setDataMode(mode));
        return unsub;
    }, []);
    ...
    // In preview mode (no live WS) and bot not already running, the Run button
        ...
        <div className={classNames('animation__wrapper', className)}>
            ...
            {isPreviewMode ? (
                <Tooltip
                    ...
                    message={localize('Connect your Deriv account to run bots with real money.')}
                    icon='info'
                    className='qs__tooltip'
                >
                    <Button
                        id='db-animation__login-button'
                    ...
                    >
                        <Localize i18n_default_text='Log in to run' />
                    </Button>
                </Tooltip>
            ) : should_show_tooltip ? (
