...
authRouter.get("/login", loginRateLimiter, (req, res) => {
...
authRouter.post("/exchange", exchangeRateLimiter, (req, res) => {
...
authRouter.get("/accounts", derivProxyRateLimiter, async (req, res) => {
...
authRouter.post("/otp", derivProxyRateLimiter, async (req, res) => {
  const authHeader = req.headers["authorization"];
  if (!authHeader?.startsWith("Bearer ")) {
    return res.status(401).json({
      error: "missing_token",
