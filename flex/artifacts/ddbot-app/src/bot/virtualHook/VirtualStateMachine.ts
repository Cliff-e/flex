...
export const VH_STATE_INFO: Record<VHState, VHStateInfo> = {
    ...
    [VHState.WAIT_FOR_ENTRY]: {
        ...
        timeoutMs: 30_000,
        failureBehavior: 'No tick within timeout: mark contract TIMED_OUT and settle as loss.',
        retryBehavior: 'No retry — the round completes as a loss.',
    },
    [VHState.ACTIVE]: {
        entryConditions: 'Entry digit captured; contract running.',
        exitConditions: 'Settlement event received or duration elapsed.',
        timeoutMs: 30_000,
        failureBehavior: 'Settlement timeout: mark contract TIMED_OUT.',
        retryBehavior: 'No retry — the round completes as a loss.',
    },
    [VHState.WAIT_FOR_EXIT]: {
        ...
        timeoutMs: 5_000,
        failureBehavior: 'Exit digit unavailable: use last known tick digit.',
        retryBehavior: 'No retry — fallback digit accepted.',
    },
    [VHState.SETTLED]: {
    ...
    [VHState.POLICY_DECISION]: {
        ...
        timeoutMs: 1_000,
        failureBehavior: 'Policy evaluation error: default to REJECTED.',
        retryBehavior: 'No retry — default to REJECTED.',
    },
...
const TRANSITIONS: Record<VHState, ReadonlySet<VHState>> = {
    ...
    [VHState.WAIT_FOR_ENTRY]: new Set([VHState.ACTIVE, VHState.SETTLED]),
    [VHState.ACTIVE]: new Set([VHState.WAIT_FOR_EXIT, VHState.SETTLED]),
