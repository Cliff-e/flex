...
export default class GoogleDriveStore {
    ...
    initialiseClient = () => {
        if (!this.client_id || !this.scope || !window.google?.accounts?.oauth2) return;
    ...
    async signIn() {
        if (!this.client) {
            botNotification(localize('Google Drive is not configured for this environment.'), undefined, {
                closeButton: true,
            });
            return;
        }
