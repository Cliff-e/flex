...
export class TradingEngine {
    ...
    // Contract locked at recovery start — never re-evaluated during a recovery session.
    private _recoveryContractType: string | null = null;
    private _recoveryBarrier: string | null = null;
    ...
    async start(): Promise<void> {
        ...
        this._recoveryContractType = null;
        this._recoveryBarrier = null;
    ...
    private handleTick(tick: any): void {
        ...
        if (this.inRecovery) {
            ...
            // The tick stream has no role while recovery is active.
            return;
        }
    ...
    private async executeBatch(): Promise<void> {
        ...
        if (!this._isStopped()) {
            if (hadLoss) {
                // Resolve the recovery contract once and lock it for the entire
                // recovery session — resolveRecoveryContract() is never called again
                // until this recovery wins and a new loss starts the next session.
                const { contractType: rcType, barrier: rcBarrier } = this.resolveRecoveryContract();
                this._recoveryContractType = rcType;
                this._recoveryBarrier = rcBarrier;
                this.inRecovery = true;
                this.log(`🔄 Loss detected — recovery locked: ${rcType} @ ${rcBarrier} — executing immediately`);
                this._journalNotify(
                    `🔄 [RECOVERY] Activated — ${this.config.strategy} suspended — ${rcType} @ ${rcBarrier} — immediate execution`
                );
            }
            this.executionLock = false;
    ...
    private async executeRecoveryTrade(): Promise<void> {
        ...
        // Recovery loop: keeps trading the locked contract until a win or stop.
        // _recoveryContractType / _recoveryBarrier are set once before this is
        // called (in executeBatch or _awaitRecovery) and never change mid-session.
        while (!this._isStopped()) {
            const contractType = this._recoveryContractType!;
            const barrier = this._recoveryBarrier!;
            ...
            try {
                ...
                if (result.won) {
                    ...
                    this._journalNotify(
                        `❌ [RECOVERY] LOST — exit ${result.exitDigit} — escalating to ${this.currentStake.toFixed(2)} — retrying immediately`
                    );
                }
                ...
                // Treat API error as a non-win — immediately retry rather than
                // hanging, so the martingale progression continues correctly.
                recoveryWon = false;
            }
            ...
            // ── LOSS: immediately loop for the next recovery trade ────────────
            // inRecovery stays true, _onRecoveryComplete is NOT resolved yet —
            // DIFFER_SEQUENCE must keep waiting until recovery wins.
        }

        // Bot stopped during recovery — release lock and unblock any waiting loop.
        this.executionLock = false;
        const cb = this._onRecoveryComplete;
        this._onRecoveryComplete = null;
        cb?.();
     ...
     * recovery engine without polling. Escalates the martingale stake
     * (DIFFER_SEQUENCE never calls applyMartingale on a sequence trade loss),
     * locks the recovery contract once, then fires executeRecoveryTrade()
     * immediately — identical behaviour to what executeBatch does for
     * OVER_1, UNDER_8 and DIFFER. Returns a Promise that resolves when
     * executeRecoveryTrade signals completion via _onRecoveryComplete.
     */
    private _awaitRecovery(): Promise<void> {
        return new Promise<void>(resolve => {
            this._onRecoveryComplete = resolve;
            // Escalate stake: DIFFER_SEQUENCE uses config.stake for its own
            // trades and never calls applyMartingale — do it here so the first
            // recovery trade starts at the martingale-escalated value, not base.
            this.applyMartingale(false);
            this.inRecovery = true;
            const { contractType, barrier } = this.resolveRecoveryContract();
            this._recoveryContractType = contractType;
            this._recoveryBarrier = barrier;
            this.setState('recovery');
            this.log(`🔄 Loss detected — recovery locked: ${contractType} @ ${barrier} — executing immediately`);
            this._journalNotify(
                `🔄 [RECOVERY] Activated — DIFFER_SEQUENCE suspended — ${contractType} @ ${barrier} — immediate execution`
            );
            void this.executeRecoveryTrade();
        });
    }
